...
## 🗺️ DXF Import & Path Feature

**File**: `MainViewModel.DxfFeature.cs`

Tóm tắt khả năng hiện có:

- Load DXF: `LoadDxfAdvanced(filePath)` và `BrowseAndLoadDxf()` (OpenFileDialog + validation).
- Trích xuất contour từ các entity chính: `Polylines2D`, `Lines`, `Circles`, `Arcs` → lưu thành `DxfContourInfo` (points, length, layer, center/radius, arc sweep).
- Tính toán bounds và thống kê: `DxfXMin/DxfXMax`, `DxfYMin/DxfYMax`, `DxfEntityCount`, `DxfLayerCount`, `DxfSampleCount`.
- Tạo preview SVG path trong `DxfPreviewPathData` bằng `GenerateSvgPath()` (scale → 200x200 preview, handles circles/arcs/lines/polylines).
- Robot coordinate mapping / transform settings: `DxfScaleX`, `DxfScaleY`, `DxfOffsetX`, `DxfOffsetY`, `DxfMirrorY`, `DxfOriginMode` (Min/Center/Custom).
- Run state and controls: `IsDxfRunning`, `IsDryRun`, `DxfStepMode`, progress fields (`DxfProgressCurrent/Total`) và current position/feed (`DxfCurrX/Y/Z`, `DxfCurrFeed`).
- Safety check: `ValidateDxfSafety()` — kiểm tra điểm đã biến đổi (scale/offset) so với giới hạn máy (ví dụ hard limits 0..1000mm) và log cảnh báo nếu vượt.
- Hỗ trợ xử lý vòng cung/tiệm cận góc: xác định hướng cung (clockwise), sweep degrees, và thêm điểm biên phù hợp cho bounds.
- Phụ thuộc: sử dụng `netDxf` để parse file DXF.

Gợi ý tiếp theo:
- Có thể mở rộng `DxfContourInfo` để lưu thêm meta (feed per-segment, cut order).
- Tích hợp `DxfPreviewPathData` vào UI (Dashboard) để hiển thị preview SVG/Canvas.
